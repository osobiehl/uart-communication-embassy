    ...
    W: Write,
{
    ...
    backoff_state: BackoffState,
    timer_wait: Option<impl Future<Output = ()> + 'a>,
}
    ...
    W: Write,
{
    ...
    pub async fn transmit(&mut self) -> WriteError {
        if let Some(time) = self.timer_wait {
            self.time.await;

            // await timer, then transmit
        }
        todo!("not yet done");
    }

    async fn internal_transmit(&mut self, frame: &mut [u8]) -> Result<(), WriteError> {
        if self.write.is_line_free() {
            todo!("begin exponential backoff")
        }
    ...
    T: AsyncTimer,
{
    ...
    async fn wait_for_tx_or_rx_reenable(&self) {
        let wait_for_rx_reenable = poll_fn(|ctx| self.runner.poll_rx_buf(ctx));
    ...
    pub async fn start(&mut self) -> ! {
        loop {
            ...
            if (self.rx_enabled.load(Ordering::Relaxed) == true) {
                ...
                .await;
                match event {
                    First(read_result) => self.handle_read_result(current_rx_frame, read_result),
